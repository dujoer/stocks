...
.hide { display:none; }
.filterbar b { color:#b8893b; }
.filterbar .warn { color:#b8332a; font-weight:600; }
...
function filterIndustry(name){
  // 切换行业时如果切到新行业，把明细口径切到「全部明细 700 条」与卡片金额对齐
  var changed = (window.__ind !== name);
  window.__ind = (window.__ind===name ? null : name);
  // 只有真正选中一个行业（或切换行业）且明细不在「全部明细」时，自动切到 p_all 与卡片口径对齐
  if (window.__ind && changed){
    var cur = document.querySelector('.pane:not(.hide)');
    if (!cur || cur.id !== 'p_all') switchTab('p_all');
  }
  applyFilter();
  refreshFilterBar();
}
...
// 兼容旧名（applyIndustry → applyFilter），保留对外语义
function applyIndustry(){ applyFilter(); }
function refreshFilterBar(){
  ...
  var scopeName = cur ? (cur.id==='p_all' ? '全部明细（700 条·近 1 月窗口）'
                                  ...
                                  : '全部明细') : '—';
  parts.push('<span>明细口径：<b>'+scopeName+'</b></span>');
  if (window.__ind) parts.push('<span>行业筛选：<b>'+window.__ind+'</b></span>');
  if (window.__dir && window.__dir!=='all') parts.push('<span>方向：<b>'+(window.__dir==='b'?'仅增持':'仅减持')+'</b></span>');
  // 警示：行业卡片金额口径与默认最新披露日 tab 不一致时给提示
  if (window.__ind && cur && cur.id!=='p_all'){
    parts.push('<span class=\"warn\">⚠️ 卡片金额口径为「近 1 月全部 700 条」，当前明细不在此口径下；金额与明细不可比。</span>');
  }
